...
Docs -
http://www.mambolearn.com/api/Mambo%204.6.x/_includes---feedcreator.class.php.html
http://quality.tikiwiki.org/cruisecontrol/artifacts/Tiki4/20100423145332/api/default/_lib---feedcreator---feedcreator.class.php.html
...
if (!is_empty($iTunes)) { define('ITUNES','1'); }
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		if (ITUNES==1) {
			$feed.= "<rss xmlns:itunes=\"http://www.itunes.com/dtds/podcast-1.0.dtd\" version=\"".$this->RSSVersion."\">\n";
		}else{
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			if ($this->items[$i]->author!="") {
				...
				if ($this->items[$i]->authorEmail!="") {
					...
					if (ITUNES==1) {
						$feed.= "            <itunes:author> " . htmlspecialchars($this->items[$i]->authorEmail) . " (".htmlspecialchars($this->items[$i]->author).")</itunes:author>\n";
					}
			...
			*/
			...
			if ($this->items[$i]->explicit!="") {
					$feed.= "            <itunes:explicit>".htmlspecialchars($this->items[$i]->explicit)."</itunes:explicit>\n";
			}
			if ($this->items[$i]->subtitle!="") {
					$feed.= "            <itunes:subtitle>".htmlspecialchars($this->items[$i]->subtitle)."</itunes:subtitle>\n";
			}
			if ($this->items[$i]->summary!="") {
					$feed.= "            <itunes:summary>".htmlspecialchars($this->items[$i]->summary)."</itunes:summary>\n";
			}
			if ($this->items[$i]->duration!="") {
					$feed.= "            <itunes:duration>".htmlspecialchars($this->items[$i]->duration)."</itunes:duration>\n";
			}
			if ($this->items[$i]->keywords!="") {
					$feed.= "            <itunes:keywords>".htmlspecialchars($this->items[$i]->keywords)."</itunes:keywords>\n";
			}
			/*   iTunes   */
